...
export default function CrenoPagePublique() {
  ...
  useEffect(() => {
    ...
    /* L'assistant est un composant React (AssistantWidget) : on ne manipule plus
       le DOM ici, on lui demande simplement de s'ouvrir. */
    window.toggleAssistant = function toggleAssistant() {
      window.dispatchEvent(new CustomEvent("assistant:ouvrir"));
    }
  ...
  * { box-sizing: border-box; }
  ...
  /* ---------- Assistant IA ---------- */
  /* Pastille d'appel : etiquette lisible sur grand ecran, pastille ronde sur mobile. */
  .assistant-fab {
    position: fixed; bottom: 24px; right: 24px; z-index: 40;
    display: flex; align-items: center; gap: 10px;
    padding: 8px 16px 8px 8px; border: none; border-radius: 999px; cursor: pointer;
    background: var(--card); color: var(--ink);
    box-shadow: 0 14px 30px -10px rgba(18,36,47,0.35);
    font-family: inherit; text-align: left;
    transition: transform .18s ease, box-shadow .18s ease;
  }
  .assistant-fab:hover { transform: translateY(-2px); box-shadow: 0 18px 36px -12px rgba(18,36,47,0.45); }
  .assistant-fab:focus-visible { outline: 3px solid var(--primary); outline-offset: 3px; }
  .assistant-fab-avatar {
    width: 42px; height: 42px; border-radius: 50%; flex-shrink: 0;
    display: flex; align-items: center; justify-content: center;
    background: var(--primary); color: #fff;
  }
  .assistant-fab-avatar svg { width: 22px; height: 22px; }
  .assistant-fab-label { display: flex; flex-direction: column; line-height: 1.25; }
  .assistant-fab-label-top { font-size: 13px; font-weight: 700; }
  .assistant-fab-label-sub { font-size: 11.5px; color: var(--ink-soft); }
  .assistant-fab.est-ouvert .assistant-fab-label { display: none; }
  .assistant-fab.est-ouvert { padding: 8px; }

  /* Panneau de discussion : hauteur bornee, corps seul defilant. */
  .assistant-panel {
    position: fixed; bottom: 92px; right: 24px; z-index: 41;
    width: 360px; max-width: calc(100vw - 32px);
    height: 470px; max-height: calc(100vh - 140px);
    display: flex; flex-direction: column; overflow: hidden;
    background: var(--card); border: 1px solid var(--line); border-radius: 18px;
    box-shadow: 0 28px 60px -22px rgba(18,36,47,0.5);
  }
  ...
  .assistant-panel-title { margin: 0; font-size: 14px; font-weight: 700; }
  .assistant-panel-subtitle { margin: 0; font-size: 11.5px; color: var(--ink-soft); }
  ...
  .assistant-msg-row { display: flex; align-items: flex-end; gap: 8px; }
  ...
  .assistant-msg-bubble {
    ...
    background: rgba(18,36,47,0.05); font-size: 13px; line-height: 1.5;
    overflow-wrap: anywhere;
  }
  .assistant-msg-user {
    background: var(--primary); color: #fff; border-radius: 14px 14px 4px 14px;
  }
  .assistant-msg-para { margin: 0 0 6px; }
  .assistant-msg-para:last-child { margin-bottom: 0; }
  .assistant-msg-list { margin: 0 0 6px; padding-left: 18px; }
  .assistant-msg-list:last-child { margin-bottom: 0; }
  .assistant-msg-list li { margin-bottom: 3px; }

  /* Attente : trois points, pas de barre de progression mensongere. */
  .assistant-msg-attente { display: flex; gap: 4px; align-items: center; padding: 12px; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .assistant-msg-attente span { animation: none; opacity: .5; }
  ...
  @media (max-width: 560px) {
    .assistant-fab { right: 16px; bottom: 16px; padding: 8px; }
    .assistant-fab-label { display: none; }
    .assistant-panel { right: 16px; left: 16px; bottom: 82px; width: auto; max-width: none; }
  }
  ...
  <AssistantWidget mode="public" title="Assistant IA" />
    </>
  );
}